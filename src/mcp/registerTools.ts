...
export function registerTools(server: McpServer) {
  ...
  server.registerTool(
    ...
    async (args: { uid?: string; pdf_path?: string; poll_interval_ms?: number; max_wait_ms?: number; join_with?: string }) => {
      try {
        ...
        if (!pdfPath) throw new ToolError({ code: "invalid_argument", message: "Either uid or pdf_path is required.", retryable: false });
  ...
  server.registerTool(
    ...
    async (args: { uid?: string; image_path?: string; poll_interval_ms?: number; max_wait_ms?: number }) => {
      try {
        const uid = String(args.uid || "").trim();
        if (uid) {
  ...
  server.registerTool(
    "doc2x_debug_config",
    { description: "Debug helper: return resolved config and API key source for troubleshooting.", inputSchema: {} },
    async () => {
    try {
      return asJsonResult({
